...
class TestOpenInVsCodeI18n:
    ...
    def test_open_in_vscode_key_count(self):
        """open_in_vscode key must appear exactly once per locale (15 total)."""
        src = I18N.read_text(encoding="utf-8")
        count = src.count("open_in_vscode:")
        assert count == 16, (
            f"Expected 15 open_in_vscode: entries (one per locale), found {count}"
        )

    def test_open_in_vscode_failed_key_count(self):
        """open_in_vscode_failed key must appear exactly once per locale (15 total)."""
        src = I18N.read_text(encoding="utf-8")
        count = src.count("open_in_vscode_failed:")
        assert count == 16, (
            f"Expected 15 open_in_vscode_failed: entries (one per locale), found {count}"
        )
